...
(function(){
  ...
  // Plain-text preview of a deck's first flashcard question, used on the
  // deck card face. Returns "" if the deck has no cards or no text content yet.
  function firstCardQuestionText(item){
    if (!item.cards || !item.cards.length) return "";
    const card = item.cards[0];
    if (!card || !card.question || !card.question.elements) return "";
    const textEl = card.question.elements.find(e=> e.type === "text" && e.content && e.content.trim());
    return textEl ? textEl.content.trim() : "";
  }

  function cardWaveSVG(color){
    return '<svg class="deck-card-wave" viewBox="0 0 160 60" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">'
      + '<path d="M0 34 C 24 6, 46 54, 72 30 C 92 12, 110 30, 130 24 C 142 20, 152 26, 160 22 L160 60 L0 60 Z" fill="' + color + '" opacity="0.4"/></svg>';
  }
  ...
  function buildDeckCard(item, opts){
    ...
    const face = el("div","deck-card-face" + (isFolder ? " deck-card-face--folder" : " deck-card-face--deck"));
    if (isFolder){
      face.innerHTML = folderFaceSVG(item.color);
    } else {
      face.style.setProperty("--card-accent", item.color);
      const q = firstCardQuestionText(item);
      if (q){
        const qEl = el("div","deck-card-question");
        qEl.textContent = q;
        face.appendChild(qEl);
      }
      face.insertAdjacentHTML("beforeend", cardWaveSVG(item.color));
  ...
  /* toggles between "select all visible" and "deselect all", flipping
     the button's own label based on whether everything is already selected */
  function toggleSelectAll(context){
    const items = context === "library" ? getVisibleLibraryItems() : getVisibleTrashItems();
    const allSelected = items.length > 0 && items.every(it=> selection.ids.has(it.id));
    if (allSelected) selection.ids.clear();
    else items.forEach(it=> selection.ids.add(it.id));
    if (context === "library") renderHome(); else renderTrash();
    updateSelectionToolbar();
  }
  function updateSelectionToolbar(){
    ...
    const trashSelectAllBtn = $("#trash-selection-toolbar [data-action='select-all']");
    if (trashSelectAllBtn){
      const items = getVisibleTrashItems();
      const allSelected = active && selection.context === "trash" && items.length > 0 &&
        items.every(it=> selection.ids.has(it.id));
      trashSelectAllBtn.textContent = allSelected ? "Deselect all" : "Select all";
    }
    refreshChrome();
  }
  ...
  /* ---------------- create folder / flashcard modal ----------------
     Shared centered dialog used by both fab-menu actions. For a folder,
     Create makes the folder immediately. For a flashcard, Create just
     opens the editor (pre-named); the deck itself isn't added to the
     grid until the person actually hits Save inside the editor. */
  ...
  function openCreateModal(type){
    ...
    if (type === "folder"){
      ...
      input.placeholder = "Flashcard name";
    }
    input.value = "";
    $("#create-modal-scrim").classList.remove("hidden");
    $("#create-modal").classList.remove("hidden");
    requestAnimationFrame(()=> input.focus());
  }
  function closeCreateModal(){
    pendingCreateType = null;
    $("#create-modal-scrim").classList.add("hidden");
    $("#create-modal").classList.add("hidden");
  }
  function confirmCreateModal(){
    const type = pendingCreateType;
    if (!type) return;
    const name = $("#create-modal-input").value.trim();
    closeCreateModal();
    if (type === "folder"){
      const folder = newFolder(name || null, currentFolder ? currentFolder.id : null);
      folders.push(folder);
      renderAll();
      showView("home");
      showToast("Folder created");
    } else {
      openEditor({ mode:"new", folderId: currentFolder ? currentFolder.id : null, name: name || null });
    }
  }
  $("#create-modal-cancel").addEventListener("click", closeCreateModal);
  $("#create-modal-scrim").addEventListener("click", closeCreateModal);
  $("#create-modal-create").addEventListener("click", confirmCreateModal);
  $("#create-modal-input").addEventListener("keydown", (e)=>{
    if (e.key === "Enter"){ e.preventDefault(); confirmCreateModal(); }
    else if (e.key === "Escape"){ e.preventDefault(); closeCreateModal(); }
  });
  ...
  function openEditor(opts){
    ...
    if (opts.mode === "new"){
      deck = { id:null, name: opts.name || "Untitled", color: COLORS[(decks.length+folders.length) % COLORS.length], favorite:false, folderId: opts.folderId || null };
      cards = [ newCard() ];
      cardIndex = 0;
    } else {
      deck = opts.deck;
  ...
  function buildElementNode(elem, side){
    ...
    if (elem.type === "text"){
      ...
      contentNode.addEventListener("blur", ()=>{
        ...
        setElementEditingUI(wrap, false);
      ...
      contentNode.addEventListener("dblclick", (e)=>{
        ...
        e.stopPropagation();
        startEditingText(wrap, contentNode);
      });
  ...
  function buildElementToolbar(elem, side){
    ...
    if (elem.type === "text"){
      ...
      editBtn.innerHTML = '<svg class="icon" viewBox="0 0 24 24" fill="none"><path d="M4 20h4l11-11-4-4L4 16v4Z" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/></svg>';
      editBtn.title = "Edit";
      editBtn.addEventListener("click",(e)=>{
        e.stopPropagation();
        const elWrap = bar.closest(".el");
        const textNode = elWrap ? elWrap.querySelector(".el-text") : null;
        if (elWrap && textNode) startEditingText(elWrap, textNode);
      });
      bar.appendChild(editBtn);
    }

    const dupBtn = el("button");
    dupBtn.dataset.role = "copy";
  ...
  /* while a text element is actively being typed into, its floating
     toolbar is hidden entirely — it reappears (with all 3 icons) once
     editing ends and the element is selected again */
  function setElementEditingUI(wrap, editing){
    const bar = wrap.querySelector(".el-toolbar");
    if (!bar) return;
    bar.classList.toggle("hidden", editing);
  }
  function startEditingText(wrap, contentNode){
    contentNode.contentEditable = "true";
    contentNode.classList.add("editing");
    contentNode.focus();
    placeCaretAtEnd(contentNode);
    setElementEditingUI(wrap, true);
  }
    ...
    $("#pane-" + side).addEventListener("click", (e)=>{
      ...
      if (ed.tool === "text"){
        ...
        requestAnimationFrame(()=>{
          ...
          if (wrap && textNode) startEditingText(wrap, textNode);
        });
